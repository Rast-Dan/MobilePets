...
fun ScreenContent(startDestination: String) {
    ...
    NavHost(
        ...
        composable(route = Navigation.Detail.route + "?petJson={petJson}") { backStackEntry ->
            val petJson = URLDecoder.decode(
                backStackEntry.arguments?.getString("petJson"),
                "utf-8")
            val pet: Pet = Gson().fromJson(petJson, Pet::class.java)

            DetailScreen(navController = navController, pet)
        }
    }
}
